...
stackinfo = readtable([ animaldrive ':\Jenni\' animal '\Zstackinfopersite.xlsx']);
...
for ii = 2:length(preprocessedrois)
...
if ~exist([animaldrive ':/Jenni/' animal '/roistructure/socialroisdata.mat'])
    for fn = 2:length(analysefilelistind)
        close all
        clearvars -except animaldrive stackinfo animal manualroisdone datatable angieind fn analysefilelistind behavindnb behaviorfileind behaviorfile lowframeind preprocessedrois ...
        ...
        % time normalized by the bseline of the trial
        for cc = 1:size(TC,1)
            ...
            for ii = 1:length(correctedstim)
                ...
                roiMatevokedstim(cc,ii,:,:) = squeeze(roiMatevoked{cc}(find(correctedstimseq == correctedstim(ii)),:))./median(squeeze(roiMatevoked{cc}(find(correctedstimseq == correctedstim(ii)),baselinewindow)),2);%./median(moviedata(:,:,startstim(ii):startstim(ii)+200),3);
        ...
        Socialind = find(Category == 1);
        Echoind = find(Category == 2);
        Noise = find(Category == 4);
        Mouseind = find(Category == 3);
        
        concatmatevokedFMB = cell2mat(cellfun(@(x) x(:,Socialind,:,:),{roiMatevokedstim},'UniformOutput',0)');
        concatmatevokedEcho = cell2mat(cellfun(@(x) x(:,Echoind,:,:),{roiMatevokedstim},'UniformOutput',0)');
        % correct by the mean baseline so all the trace is around 0
        ...
        % compute the median of the response window
        ...
        peakrespFMB = squeeze(max(permute(medianconcatmatevokedFMBBcorr(:,:,responsewindow),[3,1,2])));
        peakrespEcho =  squeeze(max(permute(medianconcatmatevokedEchoBcorr(:,:,responsewindow),[3,1,2])));
        arearespFMB = squeeze(trapz(permute(medianconcatmatevokedFMBBcorr(:,:,responsewindow),[3,1,2])));
        arearespEcho =  squeeze(trapz(permute(medianconcatmatevokedEchoBcorr(:,:,responsewindow),[3,1,2])));
        % do selectivity index
        MedindexSelect = (abs(medrespFMB)-abs(medrespEcho))./(abs(medrespFMB)+abs(medrespEcho));
        PeakindexSelect = (abs(peakrespFMB)-abs(peakrespEcho))./(abs(peakrespFMB)+abs(peakrespEcho));
        AreaindexSelect = (abs(arearespFMB)-abs(arearespEcho))./(abs(arearespFMB)+abs(arearespEcho));
        
        % Same for shuffle
        tmpconcamat = cat(2,concatmatevokedFMB,concatmatevokedEcho);
        for cc = 1:size(concatmatevokedFMB,1)
            clear tmppsth
            for ll = 1:size(tmpconcamat,3) % nb of repetition
                tmppsth{ll} = tmpconcamat(cc,datasample([1:max(Echoind)],max(Echoind),'Replace',false),datasample([1:size(tmpconcamat,3)],size(tmpconcamat,3),'Replace',false),:);
            end
        ...
        shuffleconcatmatevokedFMBBcorr = shuffleconcatmatevokedFMB-mean(shuffleconcatmatevokedFMB(:,:,:,baselinewindow),4);
        shuffleconcatmatevokedEchoBcorr = shuffleconcatmatevokedEcho-mean(shuffleconcatmatevokedEcho(:,:,:,baselinewindow),4);
        ...
        shufflepeakrespFMB = squeeze(max(permute(shufflemedianconcatmatevokedFMBBcorr(:,:,responsewindow),[3,1,2])));
        shufflepeakrespEcho =  squeeze(max(permute(shufflemedianconcatmatevokedEchoBcorr(:,:,responsewindow),[3,1,2])));
        shufflearearespFMB = squeeze(trapz(permute(shufflemedianconcatmatevokedFMBBcorr(:,:,responsewindow),[3,1,2])));
        shufflearearespEcho =  squeeze(trapz(permute(shufflemedianconcatmatevokedEchoBcorr(:,:,responsewindow),[3,1,2])));
        %selectivity index
        shuffleMedindexSelect = (abs(shufflemedrespFMB)-abs(shufflemedrespEcho))./(abs(shufflemedrespFMB)+abs(shufflemedrespEcho));
        shufflePeakindexSelect = (abs(shufflepeakrespFMB)-abs(shufflepeakrespEcho))./(abs(shufflepeakrespFMB)+abs(shufflepeakrespEcho));
        shuffleAreaindexSelect = (abs(shufflearearespFMB)-abs(shufflearearespEcho))./(abs(shufflearearespFMB)+abs(shufflearearespEcho));
        ...
        socialroisdata(fn).MedindexSelect = MedindexSelect;
        socialroisdata(fn).shuffleMedindexSelect = shuffleMedindexSelect;
        socialroisdata(fn).PeakindexSelect = PeakindexSelect;
        socialroisdata(fn).shufflePeakindexSelect = shufflePeakindexSelect;
        socialroisdata(fn).AreaindexSelect = AreaindexSelect;
        socialroisdata(fn).shuffleAreaindexSelect = shuffleAreaindexSelect;
        socialroisdata(fn).Category = Category;
        socialroisdata(fn).concatmatevokedFMB = concatmatevokedFMB;
        socialroisdata(fn).concatmatevokedEcho = concatmatevokedEcho;
        socialroisdata(fn).concatmatevokedFMBCorr = concatmatevokedFMBBcorr;
        socialroisdata(fn).concatmatevokedEchoCorr = concatmatevokedEchoBcorr;
        % compute in microns for that 2X and 4X are different
        
        if strcmp(stackinfo.zoomRecording{find(stackinfo.Site==siteind(analysefilelistind(fn)))},'4X')
            centroidcells = cellfun(@(x) median((x./[2.55,2.66])),roisCoord{1},'UniformOutput',0);
        elseif strcmp(stackinfo.zoomRecording{find(stackinfo.Site==siteind(analysefilelistind(fn)))},'2X')
    ...
    save([animaldrive ':/Jenni/' animal '/roistructure/socialroisdata.mat'],'socialroisdata','-v7.3') % not for RoofBuddy 2 (split in between 2 drives)
end

% load roi data structure
DataDrive = datadrivecolumn{analysefilelistind(2)}; % data drive for data, excel file is in N and stim too
...
for fn = 1:length(analysefilelistind)
    switch analysis
        case 'population'
            ...
            responsewindow = 11:20; % from sound onset
            ...
            if sum(cellfun(@length,{datarois.correctedstim})==12)>0
                corrected = find(cellfun(@length,{datarois.correctedstim})==12);
                tobecorrected = find(cellfun(@length,{datarois.correctedstim})==17); % removing the ectra stim for the not fucked up versions so I can concatenate
                bla = (cellfun(@(x) x(:,2:6,:,:),{datarois(tobecorrected).concatmatevokedFMB},'UniformOutput',0));
                concatmatevokedFMB = vertcat(vertcat(datarois(corrected).concatmatevokedFMB),vertcat(bla{:}));
                bli = (cellfun(@(x) x(:,2:6,:,:),{datarois(tobecorrected).concatmatevokedEcho},'UniformOutput',0));
                concatmatevokedEcho = vertcat(vertcat(datarois(corrected).concatmatevokedEcho),vertcat(bli{:}));
                blu = (cellfun(@(x) x(:,2:6,:,:),{datarois(tobecorrected).concatmatevokedFMBCorr},'UniformOutput',0));
                concatmatevokedFMBCorr = vertcat(vertcat(datarois(corrected).concatmatevokedFMBCorr),vertcat(blu{:}));
                blo = (cellfun(@(x) x(:,2:6,:,:),{datarois(tobecorrected).concatmatevokedEchoCorr},'UniformOutput',0));
                concatmatevokedEchoCorr = vertcat(vertcat(datarois(corrected).concatmatevokedEchoCorr),vertcat(blo{:}));
                bly = (cellfun(@(x) x(:,2:6,:,:),{datarois(tobecorrected).MedindexSelect},'UniformOutput',0));
                MedindexSelect = vertcat(vertcat(datarois(corrected).MedindexSelect),vertcat(bly{:}));
                blp =  (cellfun(@(x) x(:,2:6,:,:),{datarois(tobecorrected).shuffleMedindexSelect},'UniformOutput',0));
                shuffleMedindexSelect = vertcat(vertcat(datarois(corrected).shuffleMedindexSelect),vertcat(blp{:}));
            else
                concatmatevokedFMB = vertcat(datarois.concatmatevokedFMB);
                concatmatevokedEcho = vertcat(datarois.concatmatevokedEcho);
                concatmatevokedFMBCorr = vertcat(datarois.concatmatevokedFMBCorr);
                concatmatevokedEchoCorr = vertcat(datarois.concatmatevokedEchoCorr);
                MedindexSelect = vertcat(datarois.MedindexSelect);
                shuffleMedindexSelect = vertcat(datarois.shuffleMedindexSelect);
            end
            ...
            ax(3)=subplot(2,3,3); hold on; imagesc(xvalues,1:length(sortmedindind),squeeze(mean(squeeze(median(concatmatevokedFMBCorr(fliplr(sortmedindind'),:,:,:),3)),2))-squeeze(mean(squeeze(median(concatmatevokedEchoCorr(fliplr(sortmedindind'),:,:,:),3)),2))); colormap(redblue(100)); caxis([-0.1,0.1]);
            plot([0,0],[1,length(sortmedindind)],'k'); axis tight
            ...
            errorbar(xbar(st+1)-2,prtcechotot,std(prctecho),'k')
            b = bar(xbar(st+1)-1,prtcsocialtot,'FaceColor',[0.8 .2 0]);
            errorbar(xbar(st+1)-1,prtcsocialtot,std(prtcsocial),'k')
            ...
            for ii = 1:size(tmpconcamat,2)
                for iif = size(tmpconcamat,2):-1:1
                    for lp = 1:loopvalue
                        ...
                        vec2 = mean(squeeze(mean(roiMatevokedstim(:,iif,RadomSelectedFoil,responsewindow),4)),2);
                        w{lp} = vec1-vec2;
                        b = -(dot(w{lp},vec1) + dot(w{lp},vec2))/2;
            ...
            meandecodingpairwithin = mean(mean([mean(DecodingAccuracytonetc(1:5,1:5,:),3);mean(DecodingAccuracytonetc(6:10,6:10,:),3)]));
            stddecodingpairwithin = std(mean([mean(DecodingAccuracytonetc(1:5,1:5,:),3);mean(DecodingAccuracytonetc(6:10,6:10,:),3)]));
            meandecodingpairacross = mean(mean([mean(DecodingAccuracytonetc(1:5,6:10,:),3);mean(DecodingAccuracytonetc(6:10,1:5,:),3)]));
            stddecodingpairacross = std(mean([mean(DecodingAccuracytonetc(1:5,6:10,:),3);mean(DecodingAccuracytonetc(6:10,1:5,:),3)]));
            h = figure(); hold on;% subplot(1,2,1); hold on;
            bar(2,meandecodingpairwithin*100,'FaceColor',[0.5,0.5,0.5]);
            errorbar(2,meandecodingpairwithin*100,stddecodingpairwithin*100,'k');
            bar(4,meandecodingpairacross*100,'FaceColor',[0.7,0.3,0.8]);
            errorbar(4,meandecodingpairacross*100,stddecodingpairacross*100,'k');
            xticks([2,4]);
            xticklabels({'Within category','Across category'}); ylabel('Mean decoding accuracy [%]')
            saveas(h,[DataDrive ':\Jenni\' animal '\figure\social\Population\AllSites_MeanPairwiseDecodingAccuracy.pdf'])
            print([DataDrive ':\Jenni\' animal '\figure\social\Population\AllSites_MeanPairwiseDecodingAccuracy.svg'],'-dsvg')
            ...
            cellmedselect = num2cell(mean(MedindexSelect'));
            shufflecellmedselect = num2cell(mean(shuffleMedindexSelect'));
            
            secondstimlist = size(tmpconcamat,2):-1:1;
            % careful indices not flipped iif for 12 is still 12
            for dp = [Socialind]
                Socialindtmp = [Socialind];
                Socialindtmp(Socialindtmp==dp) = [];
                Echoindtmp = [Echoind];
                Echoindtmp(Echoindtmp==Echoind(dp)) = [];
                allsocialpairsind(dp,:) = Socialindtmp;
                allechopairsind(dp,:) = Echoindtmp;
                meansamecatweigthtmp(dp,:) = mean([(squeeze(mean(DecodingWeight(dp,allsocialpairsind(dp,:),:,:),4)));(squeeze(mean(DecodingWeight(Echoind(dp),allechopairsind(dp,:),:,:),4)))]);
                meanacrosscatweigthtmp(dp,:) = mean([(squeeze(mean(DecodingWeight(dp,Echoind,:,:),4)));(squeeze(mean(DecodingWeight(Echoind(dp),Socialind,:,:),4)))]);
                %                 allsocialtoechopairsind(dp,:) = find(ismember(secondstimlist,Socialindtmp+max(Socialind)));
            end
            %             meansamecatweigthtmp = [(squeeze(mean(DecodingWeight(1:6,allsocialpairsind,:,:),4))),(squeeze(mean(DecodingWeight(7:12,allsocialpairsind,:,:),4)))];
            %             meanacrosscatweigthtmp(dp,:,:) = (squeeze(mean(DecodingWeight(dp,1:6,:,:),4)));
            
            %             meanacrosscatweigthtmp R= (squeeze(mean(DecodingWeight(1:6,1:6,:,:),4)));
            meanacrosscatweigth = num2cell(mean(squeeze(meanacrosscatweigthtmp)));
            meansamecatweigth = num2cell(mean(squeeze(meansamecatweigthtmp)));
            meanshufflecatweigth = num2cell(squeeze(mean(mean(squeeze(mean(DecodingWeight(samestim(:,1),samestim(:,2),:,:),4)))))');
            ...
            for st = 1:nbsite
                for cc = find(siteveccells==st)
                    for cci = max(find(siteveccells==st)):-1:cc
                        ...
                        pariselect{st}(indcc,indcci) = (cellmedselect{cc}+cellmedselect{cci})/2;
                        pairsweightacross{st}(indcc,indcci) = (meanacrosscatweigth{cc}+meanacrosscatweigth{cci})/2;
                        pairsweightwithin{st}(indcc,indcci) = (meansamecatweigth{cc}+meansamecatweigth{cci})/2;
                        pairsweightshuffle{st}(indcc,indcci) = (meanshufflecatweigth{cc}+meanshufflecatweigth{cci})/2;
                        
                        if mean(MedindexSelect(cc,:))<boundarymedselect(1) || mean(MedindexSelect(cc,:))>boundarymedselect(2) && mean(MedindexSelect(cci,:))<boundarymedselect(1) || mean(MedindexSelect(cci,:))>boundarymedselect(2)
                            condipariselect{st}(indcc,indcci) =(cellmedselect{cc}+cellmedselect{cci})/2;
                        else
                            condipariselect{st}(indcc,indcci) =nan;
                        end
                        shufflepariselect{st}(indcc,indcci) = (shufflecellmedselect{cc}+shufflecellmedselect{cci});
                    end
                end
            end
            distcell = {datarois(:).dist};
            vecdist = cell2mat(cellfun(@(x) reshape(x,1,size(x,1)*size(x,2)),distcell,'UniformOutput',0));
            vecsi = cell2mat(cellfun(@(x) reshape(x,1,size(x,1)*size(x,2)),pariselect,'UniformOutput',0));
            vecwacross = cell2mat(cellfun(@(x) reshape(x,1,size(x,1)*size(x,2)),pairsweightacross,'UniformOutput',0));
            vecwwithin = cell2mat(cellfun(@(x) reshape(x,1,size(x,1)*size(x,2)),pairsweightwithin,'UniformOutput',0));
            vecwshuffle = cell2mat(cellfun(@(x) reshape(x,1,size(x,1)*size(x,2)),pairsweightshuffle,'UniformOutput',0));
            
            vecsishuffle = cell2mat(cellfun(@(x) reshape(x,1,size(x,1)*size(x,2)),shufflepariselect,'UniformOutput',0));
            vecsicondi= cell2mat(cellfun(@(x) reshape(x,1,size(x,1)*size(x,2)),condipariselect,'UniformOutput',0));
            vecdistcondi = vecdist;
            indzeronan = unique([find(vecdist==0),find(isnan(vecsi))]);
            vecsi(indzeronan) =[];
            vecwacross(indzeronan) =[];
            vecwwithin(indzeronan) =[];
            vecwshuffle(indzeronan) =[];
            vecdist(indzeronan) =[];
            indzeronancondi = unique([find(vecdistcondi==0),find(isnan(vecsicondi))]);
            vecdistcondi(indzeronancondi) = [];
            vecsicondi(indzeronancondi) = [];
            [discretebins,discreteedge] = discretize(vecdist,10);
            
            %  pairwise distance for all cells for Similarity index of SI
            %  and decoding weight
            for i = unique(discretebins)
                meansi(i) = mean(vecsi(find(discretebins==i)));
                semsi(i) = (std(vecsi(find(discretebins==i)))/(sqrt(numel(find(discretebins==i)))))*2;
                pairnb(i) = numel(find(discretebins==i));
                meanwacross(i) = mean(vecwacross(find(discretebins==i)));
                semwacross(i) = (std(vecwacross(find(discretebins==i)))/(sqrt(numel(find(discretebins==i)))))*2;
                meanwwithin(i) = mean(vecwwithin(find(discretebins==i)));
                semwwwithin(i) = (std(vecwwithin(find(discretebins==i)))/(sqrt(numel(find(discretebins==i)))))*2;
                meanwshuffle(i) = mean(vecwshuffle(find(discretebins==i)));
                semwwshuffle(i) = (std(vecwshuffle(find(discretebins==i)))/(sqrt(numel(find(discretebins==i)))))*2;
                
                shufflemeansi(i) = mean(vecsishuffle(find(discretebins==i)));
                shufflesemsi(i) = (std(vecsishuffle(find(discretebins==i)))/(sqrt(numel(find(discretebins==i)))))*2;
                meansicondi(i) = mean(vecsicondi(find(vecdistcondi<=discreteedge(i+1) & vecdistcondi>discreteedge(i))));
                semsicondi(i) = std(vecsicondi(find(vecdistcondi<=discreteedge(i+1) & vecdistcondi>discreteedge(i))))/(sqrt(numel((vecsicondi(find(vecdistcondi<=discreteedge(i+1) & vecdistcondi>discreteedge(i)))))))*2;
            end
            h=figure(); hold on; subplot(1,2,1); hold on
            axh = shadedErrorBar(unique(discretebins),meansi,semsi,{'Color',[0,0,0],'LineWidth',2},1);
            bxh = shadedErrorBar(unique(discretebins),shufflemeansi,shufflesemsi,{'Color',[0.5,0.5,0.5],'LineWidth',2},1);
            cxh = shadedErrorBar(unique(discretebins),meansicondi,semsicondi,{'Color',[0.6,0.1,0.8],'LineWidth',2},1);
            
            xticks(unique(discretebins));
            xticklabels(num2cell(round(discreteedge)));
            ylabel('Similarity Index (Selectivity Index based)'); xlabel('Distance pair [\mu]');
            %             linfit = fitlm(discretebins,vecsi,'linear');
            %             bxh = refline(linfit.Coefficients.Estimate(2),linfit.Coefficients.Estimate(1));
            dim = [.15 .65 .1 .1]; ylim([-0.2,0.2]); xlim([1,length((unique(discretebins)))]); axis square
            ...
            legend([axh.mainLine,bxh.mainLine,cxh.mainLine],{'all pairs','all pairs shuffle','significant pairs'}); legend box off
            %             % do equally sized bins
            % do this with decoding weight
            
            hold on; subplot(1,2,2); hold on
            dxh = shadedErrorBar(unique(discretebins),meanwacross,semwacross,{'Color',[0.6,0.1,0.8],'LineWidth',2},1);
            exh = shadedErrorBar(unique(discretebins),meanwwithin,semwwwithin,{'Color',[0.8,0.2,0.6],'LineWidth',2},1);
            fxh = shadedErrorBar(unique(discretebins),meanwshuffle,semwwshuffle,{'Color',[0.5,0.5,0.5],'LineWidth',2},1);
            xticks(unique(discretebins));
            xticklabels(num2cell(round(discreteedge)));
            ylabel('Similarity Index (Decoding Weight based)'); xlabel('Distance pair [\mu]');
            %             linfit = fitlm(discretebins,vecsi,'linear');
            %             bxh = refline(linfit.Coefficients.Estimate(2),linfit.Coefficients.Estimate(1));
            dim = [.15 .65 .1 .1]; ylim([-0.0001,0.0001]); xlim([1,length((unique(discretebins)))]); axis square
            %             annotation('textbox',dim,'String',['p = ' num2str(round(linfit.Coefficients.pValue(2)*10000)/10000) ' r =' num2str(round(linfit.Rsquared.Ordinary*10000)/10000)],'EdgeColor','none')
            legend([dxh.mainLine,exh.mainLine,fxh.mainLine],{'dw across cat','dw within cat','dw same stim'}); legend box off
...
%             durationtuningPSTHtmp = {Tonotopystruct.durationevoked};
%             durationtuningPSTH
            h=figure(); hold on; subplot(1,2,1); hold on;
            ...
            if strcmp(animal,'RoofBuddy2') % restrict cells to last 2 sites (6 and 7 corresponfing to 1 and 22)
                echoselectind = echoselectind(find(echoselectind>max(cumsum(nbcellssite(1:end-3)))))-max(cumsum(nbcellssite(1:end-3)));
                socialselectind = socialselectind(find(socialselectind>max(cumsum(nbcellssite(1:end-3)))))-max(cumsum(nbcellssite(1:end-3)));
                notselectiveind = notselectiveind(find(notselectiveind>max(cumsum(nbcellssite(1:end-3)))))-max(cumsum(nbcellssite(1:end-3)));
            end
            h=figure(); hold on; subplot(1,2,1); hold on;
