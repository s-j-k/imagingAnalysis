...
redTmp=reshape(TCpretone_reorder,[100,20*15,numcells]);
redTmpNorm=redTmp-median(redTmp(1:10,:,:)); % subtract baseline
redTmpNormRe=reshape(redTmpNorm,[100,20,15,numcells]);
...
[bfValue,bfPureTones]=max(squeeze((mean(medRepetition(20:50,:,:)))));
...
for pp = 1:(nTones-3)
   ...
   meanBFTC(pp,:)=max(squeeze(mean(medRepetition(20:50,:,tempBFIdx),3))); 
...
%         timeAxis = (0:(nTones*25)-1) * nPlanes / frameRate; % by the modulus value
        ...
        toneColor Beha= jjet(colormapIndex,:);
        for k = 1:tempNTones
