...
disk = 'Q:\Fangchen\tiling\';
disk = 'S:\jenni\';

suite2ppath = [disk mouse '\' site '\suite2p\'];
h5path = [disk mouse '\' site '\'];
...
% Set parameters
nTones = 17; %change for half-octaves
...
%Note: we are presenting each 17 tones 10 times each to the animal (quarter-octaves)
...
for i=1:nPlanes 
    ...
    if strcmp(file,'suite2p')   % use suite2p ROIs
        ...
        %Append 1 row of zeros to make both planes have the same number of frames
        if i == 2
            tc = [tc zeros(size(tc,1), 1)];
        end
...
%         ok = data.iscell(:,1); %Obtain iscell vector that index all the relevant ROIs
        ...
        TC = [TC tc']; % Get tuning curve for each neuron and concatenate to TC vector

        neuronEachPlane(i) = size(tc,1);
...
%use first 7 frames if peakFrames index is out of bound 
if startFrame < 1
    endFrame = endFrame - startFrame + 1;
    startFrame = 1;
end
