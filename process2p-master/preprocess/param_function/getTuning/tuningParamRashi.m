...
nTones = length(tone);
nTrials = 10;
nFramesPerTone = 100/nPlanes; % this is the modulus value 
nFramesPerTrial = nFramesPerTone * nTones; % 850
startTrial = 1; % the first tone is on frame 0
nFrames = nFramesPerTrial*nTrials; % 4250
frameRate = 31.25/nPlanes; % in the future, do not hard code this.
% ----------------- Tone Presentation Protocol ---------------------
pretoneFrames = 10;
baselineFrames = 5;

toneOnset = 20/nPlanes; % compare value in TDT 
...
smoothWindow = 5; smoothArg = {'movmean',smoothWindow};
...
toneActSel  = 'peak';
toneActSumBin = 1:ceil(0.66 * frameRate);%toneActSumBin = 1:ceil(0.33 * frameRate);
...
significanceTestAlpha = {0.05,0.05,0.05};%significanceTestAlpha = {0.01,0.01,0.05};
snrAnalysisList = {'roc'};
