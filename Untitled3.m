datapath='P:\su\DATA\imagingData\passive\sk023\day4\suite2p\plane0 - Copy';
cd(datapath)
load('Fall.mat')

% now load each of the behavior files
behpath='P:\su\DATA\imagingData\passive\sk023\day4';
cd(behpath)
fileList = dir(fullfile(behpath, '*tonotopy*.mat'));
load(fileList.name)
tonbatimagingdata=batimagingdata;

fileList = dir(fullfile(behpath, '*Linear*.mat'));
load(fileList.name)
linbatimagingdata=batimagingdata;

fileList = dir(fullfile(behpath, '*Angie*.mat'));
load(fileList.name)
wavbatimagingdata=batimagingdata;
clear batimagingdata
%%
protocolList=["tonotopy","linSweep","wav"];

tonotopycells=0;linsweepcells=0;wavcells=0;
tonelengthframesTon=51317;tonelengthframesLin=4129;tonelengthframesWav=32155;
tonotopyframes = [1 tonelengthframesTon];
linsweepframes= [tonelengthframesTon+1 tonelengthframesTon+1+tonelengthframesLin];
wavframes=[tonelengthframesTon+2+tonelengthframesLin tonelengthframesTon+2+tonelengthframesLin+tonelengthframesWav];
for ww=1:3
    protocol=protocolList(ww);
    if protocol=='tonotopy'
        tonotopycells=F(iscell==1,tonotopyframes(1):tonotopyframes(2));
        tonstartstim = tonbatimagingdata.frametone;%1:400:44000;

            tonseqid = tonbatimagingdata.durationseq;%batimagingdata.;%repmat(1:11,1,10);
            tontoneid = tonbatimagingdata.toneseq;%[13454;4757;26907;53813;74264;76424;19026;38052;6727;9513;70000];
            tonuniquetone = unique(tontoneid);
            tonuniqueduration = unique(tonseqid); % also order of presentation

        % calculate ITIs
        tonITIid=[];
        for kk=1:length(tontoneid)
            tonITIid=tontoneid(kk)-tonseqid(kk);
        end

        tonAttid=tonbatimagingdata.att;
        tonuniqueITI=unique(tonITIid);
        tonuniqueAtt=unique(tonAttid);

            for tid = 1:length(tonuniquetone)
                for dd = 1:length(tonuniqueduration)
                    tonordertone(tid,dd,:) = intersect(find(tontoneid==tonuniquetone(tid)),find(tonseqid==tonuniqueduration(dd))); % tone id duration and trial rep
                end
            end
            % nb of repetition
            tonnbrep = (length(tontoneid)/length(tonuniquetone))/length(tonuniqueduration)/5;
%             tonnbrep=10;
            xvalues = [-10:40]/31.25;
            responsewindow = 11:15;
    elseif protocol=='Linsweep'
        linsweepcells=F((iscell==1),linsweepframes(1):linsweepframes(2));
        linstartstim = linbatimagingdata.frametone;%1:400:44000;

            linseqid = linbatimagingdata.durationseq;%batimagingdata.;%repmat(1:11,1,10);
            lintoneid = linbatimagingdata.toneseq;%[13454;4757;26907;53813;74264;76424;19026;38052;6727;9513;70000];
            linuniquetone = unique(lintoneid);
            linuniqueduration = unique(linseqid); % also order of presentation

        % calculate ITIs
        linITIid=[];
        for kk=1:length(lintoneid)
            linITIid=lintoneid(kk)-linseqid(kk);
        end

        linAttid=batimagingdata.att;
        linuniqueITI=unique(ITIid);
        linuniqueAtt=unique(Attid);

            for tid = 1:length(linuniquetone)
                for dd = 1:length(linuniqueduration)
                    linordertone(tid,dd,:) = intersect(find(lintoneid==linuniquetone(tid)),find(linseqid==linuniqueduration(dd))); % tone id duration and trial rep
                end
            end
            % nb of repetition
            linnbrep = (length(toneid)/length(uniquetone))/length(uniqueduration);
%             linnbrep=1;
            xvalues = [-10:40]/31.25;
    ...
    elseif protocol=='wav'
                wavcells=F((iscell==1),wavframes(1):wavframes(2));
        startstim = wavbatimagingdata.frametone;%1:400:44000;

            wavseqid = wavbatimagingdata.durationseq;%batimagingdata.;%repmat(1:11,1,10);
            wavtoneid = wavbatimagingdata.toneseq;%[13454;4757;26907;53813;74264;76424;19026;38052;6727;9513;70000];
            wavuniquetone = unique(wavtoneid);
            wavuniqueduration = unique(wavseqid); % also order of presentation

        % calculate ITIs
        wavITIid=[];
        for kk=1:length(wavtoneid)
            wavITIid=wavtoneid(kk)-wavseqid(kk);
        end

        wavAttid=wavbatimagingdata.att;
        wavuniqueITI=unique(wavITIid);
        wavuniqueAtt=unique(wavAttid);

            for tid = 1:length(wavuniquetone)
                for dd = 1:length(wavuniqueduration)
                    wavordertone(tid,dd) = intersect(find(wavtoneid==wavuniquetone(tid)),find(wavseqid==wavuniqueduration(dd))); % tone id duration and trial rep
                end
            end
            % nb of repetition
            wavnbrep = (length(wavtoneid)/length(wavuniquetone))/length(wavuniqueduration);
%             wavnbrep=1;
            xvalues = [-10:40]/31.25;
...
%% now go through each protocol and analyze the data
TC=[];
ww=1;
clear roiMatevoked startstim roiDummy
for ww=1:3
    if ww==1
        batimagingdata=tonbatimagingdata;
        TC=tonotopycells;
        startstim=tonstartstim;      %  startstim=startstim(1:end-1);
        seqid=tonseqid;
        toneid=tontoneid;
        uniquetone=tonuniquetone;
        uniqueduration=tonuniqueduration;
        ITIid=tonITIid;
        Attid=tonAttid;
        uniqueITI=tonuniqueITI;
        uniqueAtt=tonuniqueAtt;
        ordertone=tonordertone;
        nbrep=tonnbrep;
    ...
    for cc = 1:size(TC,1)
        for ii = 1:length(startstim)
            if ii==length(startstim)
                % there is an error for the last startstim. not sure why.
                % says it's only 24 frames long?
                roiDummy=TC(cc,startstim(ii)-10:length(TC))./median(TC(cc,startstim(ii)-10:startstim(ii)-1),2);
                roiDummy=padarray(roiDummy,[0 16],0,'post');
                roiMatevoked(cc,ii,:) = roiDummy;
            else
...
%     colormap(flipud(gray));
    title('Tone evoked response for frequency sweeps and white noise combined')
...
%     xticklabels(mat2cell(uniquetone/1000,1,ones(1,10)))
    ylabel('df/f');
            xticklabels(string(uniquetone));   
%     saveas(h,['N:\Jenni\' animal '\figure\tonotopy\site' num2str(siteind(analysefilelistind(fn))) '\Population\' path{analysefilelistind(fn)}(end-4:end) '_Tuning.pdf'])
    ...
    h = figure(6); hold on; subplot(3,1,[1,2]); imagesc(1:length(uniqueduration),1:length(tuningrespcells),...
...
%     saveas(h,['N:\Jenni\' animal '\figure\tonotopy\site' num2str(siteind(analysefilelistind(fn))) '\Population\' path{analysefilelistind(fn)}(end-4:end) '_DurationTuning.pdf'])
    %% now order the cells by duration
    [sortbd,maxdurind] = max(durationevoked');
    [rankedsortbd,rankedmaxdur]=sort(maxdurind);
    
    h = figure(7); hold on; subplot(3,1,[1,2]);
    imagesc(1:length(uniqueduration),1:length(tuningrespcells),...
        durationevoked(rankedmaxdur,:)); colormap cool(10); colorbar %caxis([1,1.1])
    xticks([1:length(uniqueduration)]);
    xticklabels(mat2cell(uniqueduration,1,ones(1,length(uniqueduration))))
    ylabel('Significantly tuned ROIs');
...
% sort the heatmap of the ROIs and duration by max amplitude for each
% duration
...
% scatter plot of best freq by best 
