function sbx2h5bat(fname,varargin)
...
while(~done && to_read>0)
    try
        ...
        if(k==0)
            h5create(fnh,'/data',[(size(z,3)-y_crop_start)+1 size(z,2) Inf],'DataType','uint16','ChunkSize',[(size(z,3)-y_crop_start)+1 size(z,2) to_read]);
            h5write(fnh,'/data',q,[1 1 1],[(size(z,3)-y_crop_start)+1 size(z,2) to_read]);
%             f = waitbar(0,'Converting to hdf5');
        else
            h5write(fnh,'/data',q,[1 1 k+1],[(size(z,3)-y_crop_start)+1 size(z,2) to_read]);
        end
    catch
        done = 1;
%         delete(f);
