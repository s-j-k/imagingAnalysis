...
animal = 'sk023';%'RoofBuddy1';
datatable = readtable(['H:\su\' animal '\processed_sjk.xlsx']);
...
for fn = 1:length(analysefilelistind)
    close all
    clear TC durationind durationevoked durationevoked roiMatevoked ordertone depthvalue Pval stats tcnorm tunedcells tuningrespcells signicelltuning meantuningdf roiMatevokedstim meanbaselinedf dfmaxvalue dfmaxind mediandeppthrois depthroi mediantuningdf stdtuningdf
    % read frames from h5
    h5list = dir([datadrivecolumn{analysefilelistind(fn)} ':\su\' animal '\' path{analysefilelistind(fn)} '\31.25\' '*.h5']);
    nbframestmp(1) = 0;
    ...
    % load behavior file
    load([datadrivecolumn{analysefilelistind(fn)} ':\Jenni\' animal '\behavior\' behaviorfile{tonotopyind(fn)} '.mat']); %RoofBuddy1tonotopy02-03-2021-12-50
    ...
    % shadedErrorBar(1:length(uniquetone),mean(meantuningdf{ccs}'),std(meantuningdf{ccs}'),{'Color',[0,0,0]});
    % end% plot all cells signi tuning
    % first sort by bf
    ...
    h = figure(3); hold on; subplot(3,1,[1,2]); imagesc(1:length(uniquetone),...
        1:length(tuningrespcells),cell2mat(cellfun(@(x) mean(x'),...
        meantuningdf(tuningrespcells(sortbfind)),'UniformOutput',0)')); colormap gray; %caxis([1,1.1])
    ...
    h = figure(6); hold on; subplot(3,1,[1,2]); imagesc(1:length(uniqueduration),1:length(tuningrespcells),durationevoked); colormap gray; %caxis([1,1.1])
