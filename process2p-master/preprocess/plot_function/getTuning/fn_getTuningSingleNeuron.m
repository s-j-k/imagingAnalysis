...
tempF = figure; C = colormap('jet'); close(tempF); 
colormapIndex = round(linspace(1,size(C,1),nTones));
...
for j = 1:nPlanes
    for i = 1:neuronEachPlane(j)
        ...
        subplot(2,2,1)
%         rotatedrefImg=imrotate(refImg{j},-270);  
%         rotatedrefImg=flip(rotatedrefImg);
        ...
        subplot(4,2,4)
        tempSampleRate = 1;
        timeAxis = (0:tempSampleRate:nFramesPerTrial-1) * nPlanes / frameRate;
...
%         timeAxis = (0:(nTones*25)-1) * nPlanes / frameRate; % by the modulus value
        tempNTones = 20; jjet = jet; colormapIndex = round(linspace(1,size(jjet,1),20)); 
        toneColor = jjet(colormapIndex,:);
        for k = 1:tempNTones
%             tempIdx =((k-1)*100+1):(k*100);
            tempIdx =((k-1)*25+1):(k*25);
            p1 = plot(timeAxis(tempIdx), trialMedianTrialTC(tempIdx,cellIndex),'LineWidth',1.5,'color',toneColor(k,:));
        end
        %p1 = plot(timeAxis, trialMedianTrialTC(:,cellIndex),'LineWidth',1.2,'color',[0.0000 0.4470 0.7410]);
%         plot(timeAxis, trialMedianTrialTC(:,cellIndex),'LineWidth',1.2,'color',[0.0000 0.4470 0.7410]);
%         plot(timeAxis, trialMeanTrialTC(:,cellIndex),'LineWidth',1.2,'color',[0.8500 0.3250 0.0980]);
...
%         onsetTime = (1:nFramesPerTone:nFramesPerTrial) * nPlanes / 30;
        onsetTime = (1:nFramesPerTone:nFramesPerTrial) * nPlanes / frameRate; % divide by the frame rate
        for k = 1:nTones
        ...
        print(tuningFig,[savePath ...
            '/singleNeuron/Neuron' num2str(cellIndex,'%03d')], '-dpdf','-bestfit');
